 ...
 */
class seaValidatorDate extends sfValidatorBase
{
  /**
  ...
  protected function doClean($value)
  {
    ...
    else
    {
      try
      {
        $dateaux = substr($value, -4).substr($value, 3, 2).substr($value, 0, 2);
      ...
      {
        throw new sfValidatorError($this, 'Fecha no válida', array('value' => $value));
      }
    }
    ...
    if ($max = $this->getOption('max'))
    {
      ...
      {
        //$dateMax  = new DateTime(strtotime($max));
    ...
    if ($min = $this->getOption('min'))
    {
      ...
      {
        //$dateMin  = new DateTime(strtotime($max));
